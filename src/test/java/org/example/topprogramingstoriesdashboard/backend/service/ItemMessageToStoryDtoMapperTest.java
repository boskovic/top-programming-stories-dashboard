...
public class ItemMessageToStoryDtoMapperTest {
    ...
    private final ItemType TYPE = ItemType.COMMENT;
    ...
    private final Set<Ranking> RANKINGS = Set.of(Ranking.values());


    @Test
    public void mapItemDtoAndStoryRankingsToItemMessage() {
        var itemMessage = new ItemMessage(
                ID, BY, DESCENDANTS, KIDS, SCORE, TIME, TITLE, TYPE, URL, DELETED, TEXT, DEAD, PARENT, POLL, PARTS, RANKINGS
        );
        var mapper = new ItemMessageToStoryDtoMapper();

        var result = mapper.map(itemMessage);
        ...
        assertSoftly(softAssertions -> {
                    ...
                    softAssertions.assertThat(result.type()).isEqualTo(StoryTypeDto.COMMENT);
